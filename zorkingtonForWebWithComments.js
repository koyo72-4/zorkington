...
// Adds chevron (not the gas station, Katia) before each user input when logged
function addChevron() {
...
// Displays current room at the bottom of the page.
...
// If you hit enter, user input is submitted into the game.
...
// If you click button, user input is submitted into the game.
button.addEventListener('click', handleClick);

function handleClick() {
    // Formatting user input and adding it into message div (turning input into output!)
    ...
    // Formatting (lower case) + differentiating between action and "noun".
    ...
    // Take action and its exceptions - you can't take sign, and granola has special properties.
    if (action === "take" && object !== "sign" && object !== "granola") {
        // Inventory and room inventory "this ain't here"/"you already have" errors.
        ...
        // If condition is met, take object like a normal person!!!
        } else {
            takeItem(object);
        }
    // If you drop an item or view inventory, do that. If any other command, foist it off on an individual room function.
...
let roomInventory = {
    // Item names overall - lower case because we're making input lower case, because users are sloppy with the shift key.
    ...
    // Rooms and their starting inventory.
    mainStreet182: [],
    foyer: ["seven days"],
    secondFloor: [],
...
// Lower case inventory used to deal with lower case input, inventory overall = what the user sees.
...
// Messages that appear when you take an item.
